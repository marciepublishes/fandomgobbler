(function (global) {
  ...
  function normalizeMarkedForLaterSyncState(raw) {
  ...
  function filterMarkedForLaterCandidates(candidates, query) {
  ...
  function parseMarkedForLaterEntry(blurb) {
  ...
  // AO3's Marked for Later page uses li.work.blurb.group, unlike bookmarks which use li.bookmark.blurb.group.
  function parseMarkedForLaterPageDocument(doc) {
    const entries = Array.from(doc.querySelectorAll('li.work.blurb.group'))
      .map(parseMarkedForLaterEntry)
      .filter(Boolean);
  ...
  function mergeKnownMarkedForLaterWorkIds(existingKnownWorkIds, fetchedWorkIds, limit) {
    const max = Number(limit) > 0 ? Number(limit) : 5000;
    return [...new Set([
      ...(Array.isArray(existingKnownWorkIds) ? existingKnownWorkIds : []),
      ...(Array.isArray(fetchedWorkIds) ? fetchedWorkIds : [])
    ])].slice(-max);
  }

  function buildTrackedWorkFromMarkedForLaterEntry(entry, options) {
    const source = entry || {};
  ...
  const core = {
    normalizeMarkedForLaterSyncState,
    filterMarkedForLaterCandidates,
    extractBlurbTextSummary,
    extractBlurbStatNumber,
    parseMarkedForLaterEntry,
    parseMarkedForLaterPageDocument,
    mergeKnownMarkedForLaterWorkIds,
    buildTrackedWorkFromMarkedForLaterEntry
  };

  global.AO3TrackerMarkedForLaterImportCore = core;
